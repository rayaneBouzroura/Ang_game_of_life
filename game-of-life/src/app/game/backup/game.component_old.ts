...
const BOARD_WIDTH = 1000; //1
const BOARD_HEIGHT = 600; //1
const RESOLUTION = 10;
...
export class GameComponent implements OnInit, AfterViewInit {
  createGrid() {
    const gridSize = this.getGridSize(this.selectedSize);
  }
  getGridSize(selectedSize: BoardSize) {
    switch (selectedSize) {
  ...
  selectedSize: BoardSize = BoardSize.Medium; // default to Medium
  rows: number = 0;
  columns: number = 0;
  ...
  updateBoardSize(size: BoardSize) {
